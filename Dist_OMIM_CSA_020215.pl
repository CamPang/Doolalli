...
foreach my $pdb_id(@common_omim_csa_pdb)
{
	...
	OMIM: foreach my $omim_row (@$omim_rows_p)
			...
			CSA: foreach my $csa_row (@$csa_rows_p) #get csa_res_atoms
				...
				printf " \n ATOM-ATOM Distance for OMIM residue %s and CSA residue %s:\n ", $omim_row->{pdb_res_num},$csa_row->{res_num}; 
				...
				print " \n Min distance:  $min_aa_dist \n ";
				print " \n Max distance:  $max_aa_dist \n ";
				print " \n COG distance:  $cogdistance \n ";
...
#sub calculate_distance_atom {
	
#my ($omim_res_atms,$csa_res_atms)= @_;  
...
#foreach my $atom (@{$omim_res_atms})
#{
#	$omim_X_coord = $atom ->{x_coord};
#        $omim_Y_coord = $atom ->{y_coord};
#	$omim_Z_coord = $atom ->{z_coord};
#}
#
#foreach my $atom (@{$csa_res_atms})
#{
#	$csa_X_coord = $atom ->{x_coord};
#        $csa_Y_coord = $atom ->{y_coord};
#	$csa_Z_coord = $atom ->{z_coord};
#}
#
#calculate distance in 3D geometry

#print " Calculating distance between the OMIM and CSA atom co-ordinates.......\n\n";
...
#my @distance_omim_csa = sqrt($total);
...
############# ignore this as need to work on #############
### LATER PYMOL STUFF ######
